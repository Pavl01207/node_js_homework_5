...
class DataFileManager {
  ...
  // Метод writeFileSync() збереження цілого переданого через параметри масиву
  saveData(dataArray) {
    try {
  ...
  // Метод зчитування readFileSync() усього масиву з файлу
  loadData() {
    try {
  ...
  addItem(item) {
    try {
      ...
      //відпарсені дані пушимо в масив
      data.push(item)
      this.saveData(data)
    } catch (err) {
  ...
  deleteItemById(id) {
    try {
      const data = this.loadData()
      //за допомогою filter отримаємо новий масив без даних із відповідним id
